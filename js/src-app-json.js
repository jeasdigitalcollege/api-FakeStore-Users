...
async function inicializarApp() {
  ...
  try {
    ...
    console.error(
      "Erro catch(error.message) inicializarApp() - Console (message):",
      error.message
    );
    console.error(
      "Erro catch(error.name) inicializarApp() - Console:",
      error.name
    );
    console.error(
      "Erro catch(error.stack) inicializarApp() - Console:",
      error.stack
    );

    // Em caso de erro na execução dos comandos do bloco "try" executa o "catch"
    mostrarAlerta("Erro catch inicializarApp() - Alert(). Verificar Console");
    //throw new Error(
    //  `Throw catch inicializarApp(). Analisar Console: ${error.message}`
    //);
  }
